...
export default function OverviewPanel({ data, titanNews, fiidii, bulkDeals, pendingEvents, marketStatus }: any) {
  return (
    <div className="space-y-10">
      <div className="grid grid-cols-4 gap-6">
        {data?.Stocks?.map((s: any) => {
          const hasEvent = pendingEvents.some((e: any) => e.ticker.toUpperCase() === s.symbol.replace('.NS', '').toUpperCase());
          return (
            <div key={s.symbol} className={cn("p-8 bg-white/5 border rounded-[2.5rem] relative group overflow-hidden transition-all", hasEvent ? "border-rose-500/50" : "border-white/10")}>
              {hasEvent && <div className="absolute inset-x-0 top-0 py-1.5 bg-rose-500 text-white text-[8px] font-black uppercase tracking-tighter flex items-center justify-center space-x-1"><AlertTriangle size={8}/><span>TACTICAL LOCK — DO NOT TRADE</span></div>}
      ...
      <div className="grid grid-cols-3 gap-8">
        <div className="col-span-2 space-y-4">
          <h2 className="text-xs font-black uppercase text-gray-500 px-4">Institutional Flow Tracker (Bulk & Block)</h2>
        ...
        <div className="space-y-4">
          <h2 className="text-xs font-black uppercase text-gray-500 px-4">Strategic News Feed</h2>
          <div className="space-y-4">
            {titanNews.map((n: any, i: number) => (
