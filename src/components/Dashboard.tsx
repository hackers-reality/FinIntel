      ...
      <main className="pt-28 pb-20 px-10 max-w-7xl mx-auto">
        {activeTab === 'overview' && (
           <div className="space-y-10">
                 ...
                 {data?.Stocks?.map((s: any) => {
                   ...
                   return (
                     <div key={s.symbol} className={cn("p-8 bg-white/5 border rounded-[2.5rem] relative group overflow-hidden transition-all", hasEvent ? "border-rose-500/50 shadow-[0_0_30px_#f43f5e10]" : "border-white/10")}>
                       {hasEvent && <div className="absolute inset-x-0 top-0 py-1.5 bg-rose-500 text-white text-[8px] font-black uppercase tracking-tighter flex items-center justify-center space-x-1"><AlertTriangle size={8}/><span>DO NOT TRADE</span></div>}
              ...
              <div className="grid grid-cols-3 gap-8">
                 ...
                 <div className="space-y-4">
                    <h2 className="text-xs font-black uppercase text-gray-500 px-4">Titan Sentinel (X & Blogs)</h2>
                    <div className="space-y-4">
                       {titanNews.map((n, i) => (
        ...
        {activeTab === 'research' && (
           <div className="grid grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                 <textarea placeholder="Paste fine print..." className="w-full h-48 bg-white/5 border border-white/10 rounded-3xl p-6 text-sm outline-none resize-none" value={docText} onChange={e => setDocText(e.target.value)} />
