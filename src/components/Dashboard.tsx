...
const Dashboard: React.FC = () => {
  ...
  const exportReport = () => {
    ...
    const report = `# Strategic Report: ${selectedTicker}\n\nVerdict: ${researchResult.Verdict}\nScore: ${researchResult.Score}%\nTarget Price: ${researchResult.Target_Price}\nMoat Score: ${researchResult.Moat_Score}/10\n\n## Summary\n${researchResult.Summary}\n\n## Identified Risks\n${(researchResult.Risks || []).map((r: string) => `- ${r}`).join('\n')}\n\n---\nGenerated by FinIntel Pro`;
    navigator.clipboard.writeText(report);
    alert("Strategic Report copied to clipboard as Markdown.");
  };
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex justify-between items-center bg-black/20 p-6 rounded-[2rem] border border-white/5">
               <div className="space-y-1">
                 ...
                 <h3 className="text-5xl font-black text-gradient">{researchResult.Verdict}</h3>
               </div>
               <div className="text-right space-y-1">
                 <span className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Conviction</span>
                 <div className="text-3xl font-black text-white">{researchResult.Score}%</div>
               </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="glass-panel p-6 border-white/10">
                ...
                <div className="flex items-center space-x-4">
                  <div className="flex-1 h-2 bg-white/5 rounded-full overflow-hidden">
                    <motion.div 
                      initial={{ width: 0 }}
                      animate={{ width: `${(researchResult.Moat_Score || 5) * 10}%` }}
                      className="h-full premium-gradient" 
                    />
                  </div>
                  <span className="text-lg font-black text-white">{researchResult.Moat_Score}/10</span>
                </div>
              </div>
              <div className="glass-panel p-6 border-white/10 flex flex-col justify-center items-center">
                ...
                <span className="text-2xl font-black text-emerald-400">{researchResult.Target_Price}</span>
              </div>
            </div>
            ...
            <div className="space-y-3">
              {notifications.length > 0 ? notifications.map((alert, i) => (
                <motion.div 
                  ...
                  animate={{ opacity: 1, x: 0 }}
                  className="flex items-center justify-between p-5 bg-white/5 rounded-3xl border border-white/5 hover:border-cyan-400/30 transition-all group"
                >
                  <div className="flex items-center space-x-6">
                    <div className="w-10 h-10 bg-cyan-400/10 rounded-2xl flex items-center justify-center">
